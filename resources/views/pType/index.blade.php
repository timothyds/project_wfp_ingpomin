...
@section('content')
<a class="btn btn-success" href="{{route('pType.create')}}">+ New Product Type</a>
...
<table class="table">
  ...
  <tbody>
    ...
    <tr id="tr_{{$d->id}}">
      ...
      <td>{{$d->created_at}}</td>
      <td>{{$d->updated_at}}</td>
      <td><a class="btn btn-warning" href="{{ route('pType.edit', $d->id)}}">Edit</a></td>
      <td>
        ...
        <form method="POST" action="{{route('pType.destroy',$d->id)}}">
          @csrf
          @method('DELETE')
          <input type="submit" class="btn btn-danger" onclick="return confirm('Are you sure to delete {{$d->id}}-{{$d->name}} ? ')" value="Delete" action="{{route('pType.destroy',$d->id)}}">
        </form>
        @endcan
      </td>
    </tr>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST" action="{{route('pType.store')}}">
          @csrf
